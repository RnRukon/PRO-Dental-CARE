...
const NavigationBar = () => {
    ...
    return (
        <Navbar fixed="top" className=" px-10" style={{ backgroundColor: ('rgb(0, 0, 71') }} expand="lg">
            ...
            <Navbar.Collapse id="navbarScroll">
                <Nav
                    ...
                    {!user.email ? <Nav.Link style={{ fontWeight: "bold", color: 'white' }} as={NavLink} activeStyle={{ fontWeight: "bold", color: "red" }} to="/login" >Login</Nav.Link> :
                        <Nav.Link onClick={logOut} style={{ fontWeight: "bold", color: 'white' }} activeStyle={{ fontWeight: "bold", color: "red" }} >Logout</Nav.Link>}
                    {user.email && <Nav.Link onClick={handleDeleteAccount} style={{ fontWeight: "bold", color: 'white' }} activeStyle={{ fontWeight: "bold", color: "red" }} >Delate account</Nav.Link>}
